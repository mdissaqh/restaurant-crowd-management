        ...
        <div className="flex-grow-1 p-4">
                ...
                {orders.filter(o=>o.rating!=null)
                  ...
                  .map(o=>(
                    <li key={o._id} className="list-group-item">
                      ...
                      <div><strong>CGST:</strong> ₹{(o.cgstAmount||0).toFixed(2)}</div>
                      <div><strong>SGST:</strong> ₹{(o.sgstAmount||0).toFixed(2)}</div>
                      {o.serviceType==='Delivery' && (<div><strong>Delivery Charge:</strong> ₹{(o.deliveryCharge||0).toFixed(2)}</div>)}
                      <div><strong>Total:</strong> ₹{o.total.toFixed(2)}</div>
          ...
          {activeTab === 'settings' && (
            <section>
              ...
              {['dineIn','takeaway','delivery'].map(type=>(<div className="form-check" key={type}>
                <input
                  id={`${type}Toggle`}
                  type="checkbox"
